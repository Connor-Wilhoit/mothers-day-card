...
unsigned long testText() {
    ...
    tft.setTextColor(BLUE);  tft.setTextSize(2);
    tft.println("That yee be the best");
    tft.setTextColor(BLUE);  tft.setTextSize(2);
    tft.println("Momma anyone");
    tft.setTextColor(BLUE);  tft.setTextSize(2);
    tft.println("could ask for!");
    tft.println();
    tft.setTextColor(RED);  tft.setTextSize(1);
    tft.println("I Love you! :D ");
    delay(10000);
    return micros() - start;
}
...
#define RUNTEST(n, str, test) { result[n].msg = PSTR(str); result[n].ms = test; delay(1000); }
void runtests(void)
{
    uint8_t i, len = 24, cnt;
    uint32_t total;
    RUNTEST(0, "\n\nHappy Mother's Day!\n", testFillScreen());
    RUNTEST(1, "Best Momma Eva!\n", testText());
    RUNTEST(2, "I Love You!\n", testLines(CYAN));
    RUNTEST(3, "Coolest Mom on da Block!\n", testFastLines(RED, BLUE));
    RUNTEST(4, "Mwuahh!\n", testRects(GREEN));
    RUNTEST(5, "Youngest Looking Momma!\n", testFilledRects(YELLOW, MAGENTA));
    RUNTEST(6, "Smooches\n", testFilledCircles(10, MAGENTA));
    RUNTEST(7, "Happy Mother's Day\n", testCircles(10, WHITE));
    RUNTEST(8, ":D :D :D\n", testTriangles());
    RUNTEST(9, "Love You.. :)\n", testFilledTriangles());
    RUNTEST(10, "Lots o' Love\n", testRoundRects());
    RUNTEST(11, "Love, Connor Wayne\n", testFilledRoundRects());
...
void loop(void)
{
    ...
    if (tft.height() > 64) {
      ...
      tft.vertScroll(0, maxscroll, 0);
            ...
            tft.setTextColor(YELLOW, BLACK);
            if (pixel == YELLOW) {
                printmsg(msglin, "BEST MOMMA EVA    ");
#if 0
                ...
                tft.println("Best Momma around! :D");
            else {
                tft.print("Coolest Mom in town ;) ");
            }
            delay(5000);
        }
    }
    printmsg(msglin, "BEST MOMMA EVA! :D");
